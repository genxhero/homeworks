...
def top_titles
  ...
  hash = {}
  titles = Movie.select('id', 'title').where("score >=  ?", 9)
end

def star_wars
  #display the id, title and year of each Star Wars movie in movies.
  # hint: use 'select' and 'where'
  result = Movie.select('id', 'title', 'yr').where("title LIKE 'Star Wars%'")
end
...
def below_average_years
  ...
  result = Movie.select('yr', "count(*) as bad_movies").where("score <  ?", 5).group("yr")

end

def alphabetized_actors
  # display the first 10 actor names ordered from A-Z AND THE ID'S TOO, APPARENTLY!!!
...
def pulp_fiction_actors
 ...
 result = Movie.select('actors.id', 'actors.name').joins(:actors).where("title LIKE 'Pulp Fiction'")
end
...
def uma_movies
  ...
  reesult = Movie.select('id', 'title', 'yr').joins(:actors).where("actors.name = 'Uma Thurman'")
end
