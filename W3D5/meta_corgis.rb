...
class MetaCorgiSnacks
  ...
  #     info_name = "get_#{name}_info"
  #     tasty_name = "get_#{name}_tastiness"
  #     info = @snack_box.send(info_name, @box_id)
  #     tastiness = @snack_box.send(tasty_name, @box_id)
  ...
  def self.define_snack(name)
    define_method(name) do
      ...
      info = @snack_box.send(info_name, @box_id)
      tastiness = @snack_box.send(tasty_name, @box_id)
      result = "NAME: #{name}, INFO: #{info} TASTINESS: #{tastiness} "
      tastiness > 30 ? "* #{result}" : result
    end
  end
